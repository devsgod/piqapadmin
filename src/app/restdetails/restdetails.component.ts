...
export class RestdetailsComponent implements OnInit {
  ...
  updateVenue() {
    ...
    this.cusineDP = [];
    this.selectedItemsDP.forEach(element => {
    ...
    if (format.test(this.fname)) {
      this.error('Please enter valid First name');
      return false;
    }
    if (format.test(this.lname)) {
      this.error('Please enter valid Last name');
      return false;
    }
    if (format.test(this.name)) {
      this.error('Please enter valid Restaurant name');
      return false;
    }
    // if (!this.latitude || !this.longitude || this.latitude === '' || this.longitude === '') {
    // this.error('Please select valid address');
    // return false;
    ...
    this.api.updateVenue(param).then((data) => {
      this.spinner.hide();
      console.log(data);
      this.api.alerts('Success', 'Restaurant updated successfully', 'success');
      this.navCtrl.back();
    }, error => {
      this.spinner.hide();
      console.log(error);
  ...
  create() {
    ...
    this.api.checkEmail(this.email).then((datas: any) => {
      if (!datas.length) {
        this.api.register(this.email, this.password, this.fname, this.lname).then((data) => {
          if (data && data.uid) {
            ...
            this.api.createVenue(param).then((data) => {
              this.spinner.hide();
              console.log(data);
              this.api.alerts('Success', 'Restaurant added successfully', 'success');
        ...
        this.spinner.hide();
        this.error('this email id is already register, please use another to login');
      }
    }, error => {
      console.log(error);
      this.spinner.hide();
