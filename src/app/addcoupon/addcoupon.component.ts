...
export class AddcouponComponent implements OnInit {
  ...
  create() {
    ...
    if (this.selectedItems.length > 0 && this.code && this.desc
      ...
      if (this.min < 0) {
        this.error('Wrong input for Min Cart Price');
        return false;
      }
      if (this.discout < 0 || this.upto < 0) {
        this.error('Wrong input for Discout Value and Discout Value Upto');
        return false;
      }
      const date = moment(this.date);
      console.log(date.isValid());
      if (!date.isValid()) {
        this.error('Wrong Expire date');
        return false;
      }
      const today = moment();
      console.log('status', date.isAfter(today));
      if (!date.isAfter(today)) {
        this.error('Date must be in future');
        return false;
      }
